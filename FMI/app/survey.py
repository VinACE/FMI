...
qa = {
    ...
    "hedonics" : {	
	    ...
	    "strength": (["Q6_1__Orange_flavor_strength_JAR"], strength5),
    ...
    "liking" : {
        "_liking7": (["h7_overall_liking fragrance"], liking7),
...
def col_map_answer(survey_name, column):
    ...
    questions = survey2qst[survey_name]
...
def field_map_dashboard(field):
    for dbname, db in models.SurveySeekerView.dashboard.items():
        if db['X_facet']['field'] == field:
            return dbname
        if 'Y_facet' in db:
            if db['Y_facet']['field'] == field:
                return dbname
    return None
